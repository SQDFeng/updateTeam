...
function showMsg() {
  return new Promise(async resolve => {
    ...
    try {
      await $.http.get({url: `https://purge.jsdelivr.net/gh/DX3242/updateTeam@master/shareCodes/jd_updateSmallHomeInviteCode.json`}).then((resp) => {
              ...
              if (data.body) {
               if (data.body.id) {
                  console.log(`\n您的${$.name}shareCode:【${data.body.id}】\n`);
                    $.shareCode = [...($.shareCode || []), ...[data.body.id]];
                }
              }
            }
          }
