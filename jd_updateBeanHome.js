...
function showMsg() {
  return new Promise(async resolve => {
    try {
      await $.http.get({url: `https://purge.jsdelivr.net/gh/DX3242/updateTeam@master/shareCodes/jd_updateBeanHome.json`}).then((resp) => {
          ...
          if (safeGet(data)) {
            ...
            if (!shareCode) {
              ...
              if (data['data']['beanCountProgress']['progressNotYet']===0){
                ...
                console.log(`京东账号${$.index}${$.nickName}未满员`)
                $.codeInfo.push({
                  shareCode,
                  groupCode
                })
